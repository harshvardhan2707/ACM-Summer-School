...
int main(int argc, char *argv[]){
	...
	double lx, ly, lz, xij, yij, zij, rsq, mindistsq, mindist;
	...
	if(argc<1+4){
	   printf("Error!!!\n");
	   printf("Format: %s <No.of particles> <Temperature> <lx> <Init Read - 1> <Init Conf>\n",argv[0]);
   	   exit(1);
	}
	...
	lx = atof(argv[3]);
	choice = atoi(argv[4]);
	if( choice==1 )	fpconf = fopen(argv[5],"r");
	ly = lx;
	lz = lx;
...
//**************** Initialize the system with Num_par no.of particles******************* //
	...
	if(choice==1){
	  ...
	  fclose(fpconf);
	     ...
	     mindist = 1.1; mindistsq = mindist*mindist;
     	     for(i=1; i<=num; i++){
                atomdone = 0; 
     		while( atomdone==0 ){
                     xtmp = lx*(drand48()-0.5);
			...
			rsq = xij*xij + yij*yij + zij*zij;
     			if( rsq<mindistsq )atomnotdone = 1;
     		     }
